...
void Problem::AddOrderingSLAM(std::shared_ptr<NonlinearSolver::Vertex> v) {
    if (IsPoseVertex(v)) {
        v->SetOrderingId(ordering_poses_);
        idx_pose_vertices_.insert(std::pair<unsigned long, std::shared_ptr<Vertex>>(v->Id(), v));
        ordering_poses_ += v->LocalDimension();
    } else if (IsLandmarkVertex(v)) {
        v->SetOrderingId(ordering_landmarks_);
        ordering_landmarks_ += v->LocalDimension();
        idx_landmark_vertices_.insert(std::pair<unsigned long, std::shared_ptr<Vertex>>(v->Id(), v));
    }
}
...
bool Problem::IsPoseVertex(std::shared_ptr<NonlinearSolver::Vertex> v) {
    string type = v->TypeInfo();
    return type == string("VertexPose") ||
            type == string("VertexSpeedBias");
}

bool Problem::IsLandmarkVertex(std::shared_ptr<NonlinearSolver::Vertex> v) {
    string type = v->TypeInfo();
...
bool Problem::AddEdge(shared_ptr<Edge> edge) {
    ...
    for (auto &vertex: edge->Verticies()) {
        vertexToEdge_.insert(std::pair<unsigned long, shared_ptr<Edge>>(vertex->Id(), edge));
    }
    return true;
}
...
void Problem::SetOrdering() {
    ...
    if (problemType_ == ProblemType::SLAM_PROBLEM) {
        ...
        unsigned long all_pose_dimension = ordering_poses_;
...
//        TicToc t_Hmminv;
        ...
        for (unsigned long i = 0; i < ordering_poses_; ++i) {
 ...
 *
 */
Eigen::Matrix<double, Eigen::Dynamic, 1> Problem::PCGSolver(const Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> &A, const Eigen::Matrix<double, Eigen::Dynamic, 1> &b, int maxIter = -1) {
