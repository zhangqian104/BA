...
void Edge::RobustInfo(double &drho, Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> &info) const{
    if(lossfunction_)
    {
        ...
        Eigen::Vector3d rho;
        lossfunction_->Compute(e2,rho);
		Eigen::Matrix<double, Eigen::Dynamic, 1> weight_err = information_ * residual_;

		Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> robust_info(information_.rows(), information_.cols());
