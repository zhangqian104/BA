...
void IMU::AddIMUnoise(MotionData& data)
{
    ...
    Eigen::Matrix3d gyro_sqrt_cov = param.gyroNoiseSigma * Eigen::Matrix3d::Identity();
    data.imuGyro = data.imuGyro + gyro_sqrt_cov * noise_gyro / sqrt( param.imuTimestep ) + gyroBias;

    Eigen::Vector3d noise_acc(noise(generator_),noise(generator_),noise(generator_));
    Eigen::Matrix3d acc_sqrt_cov = param.accNoiseSigma * Eigen::Matrix3d::Identity();
    data.imuAcc = data.imuAcc + acc_sqrt_cov * noise_acc / sqrt( param.imuTimestep ) + accBias;

    // gyro_bias update
    Eigen::Vector3d noise_gyro_bias(noise(generator_),noise(generator_),noise(generator_));
    gyroBias += param.gyroBiasSigma * sqrt(param.imuTimestep ) * noise_gyro_bias;
    data.imuGyroBias = gyroBias;

    // acc_bias update
    Eigen::Vector3d noise_acc_bias(noise(generator_),noise(generator_),noise(generator_));
    accBias += param.accBiasSigma * sqrt(param.imuTimestep ) * noise_acc_bias;
